...
//public class BEHAVIOR3SelectMarkovAtomic extends AtomicModelImpl implements PhaseBased,
public class BEHAVIOR3 extends AtomicModelImpl implements PhaseBased,
StateVariableBased {
	...
	// Input ports
	public final Port<Serializable> inFLOW10 = 
			addInputPort("inFLOW10", Serializable.class);
	// End input ports

	// Output ports
	public final Port<Serializable> outF_95 = 
			addOutputPort("outF_95", Serializable.class);
	public final Port<Serializable> outF_5 = 
			addOutputPort("outF_5", Serializable.class);
	//ENDID
	...
	public BEHAVIOR3(String name) {
		this(name, null);
	}

	public BEHAVIOR3(String name, Simulator simulator) {
	...
	public void initialize() {
		...
		try {
			ctm.fillTransitionInfoList(path + File.separator + "xml" + 
					File.separator + "BEHAVIOR3.xml");
			TransitionInfo ti;
			TimeInfo tf;
			ti = ctm.getTransitionInfoFor("active", "portf_95_state");
            tf = ti.getTInfo();
            //ti.setProbValue(1.0/numberOfOutputPorts);
			ti = ctm.getTransitionInfoFor("active", "portf_5_state");
            tf = ti.getTInfo();
            //ti.setProbValue(1.0/numberOfOutputPorts);
		} catch (Exception e) {
			e.printStackTrace();
	...
	public void internalTransition() {
		...
		if (phaseIs("portf_95_state")) {
			getSimulator().modelMessage("Internal transition from portf_95_state");
		...
		if (phaseIs("portf_5_state")) {
			getSimulator().modelMessage("Internal transition from portf_5_state");
	...
	public MessageBag getOutput() {
		...
		if (phaseIs("active")) {
			...
			int counter = 0;
			counter++;
			if (ctm.isOutput() && ctm.getNextState().equals("portf_95_state")) {
				//output.add(F_95,"f_95");
				output.add(outF_95,job); //Modified to preserve I/O identity
			}
			counter++;
			if (ctm.isOutput() && ctm.getNextState().equals("portf_5_state")) {
				//output.add(F_5,"f_5");
				output.add(outF_5,job); //Modified to preserve I/O identity
			}
		}
	...
	public static void main(String[] args) {
		...
		//BEHAVIOR3SelectMarkovAtomic model = new BEHAVIOR3SelectMarkovAtomic();
		BEHAVIOR3 model = new BEHAVIOR3();
		model.options = options;

		if (options.isDisableViewer()) { // Command line output only
			Simulation sim =
					new com.ms4systems.devs.core.simulation.impl.SimulationImpl("BEHAVIOR3 Simulation",
							model, options);
	...
	protected static File getModelsDirectory() {
		...
		File dir;
		try {
			//dirUri = BEHAVIOR3SelectMarkovAtomic.class.getResource(".").toURI();
			dirUri = BEHAVIOR3.class.getResource(".").toURI();
			dir = new File(dirUri);
		} catch (URISyntaxException e) {
			e.printStackTrace();
			throw new RuntimeException(
					"Could not find Models directory. Invalid model URL: " +
							//BEHAVIOR3SelectMarkovAtomic.class.getResource(".").toString());
							BEHAVIOR3.class.getResource(".").toString());
		}
	...
	public String[] getPhaseNames() {
		...
		return new String[] { "active"		,"portf_95_state"
		,"portf_5_state"
 };
	}
