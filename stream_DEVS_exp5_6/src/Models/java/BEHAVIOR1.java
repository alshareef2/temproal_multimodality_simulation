...
// this is a sync node
public class BEHAVIOR1 extends AtomicModelImpl implements PhaseBased{
	...
	public final Port<Serializable> inFLOW11 = 
			addInputPort("inFLOW11", Serializable.class);
	
	// End input ports

	// Output ports

	public final Port<Serializable> outFLOW12 = 
			addOutputPort("outFLOW12", Serializable.class);
	public final Port<Serializable> outFLOW13 = 
			addOutputPort("outFLOW13", Serializable.class);

	//ENDID

	public BEHAVIOR1(String  name,double  Processing_time){
		super(name);
		addInputPort("in");
		addOutputPort("out");
	...
	public BEHAVIOR1(String name) {
		this(name, null);
	}
	
	public BEHAVIOR1(String name, Simulator simulator) {
	...
	public void externalTransition(double e, MessageBag x) {
		...
		for (int i=0; i< x.size();i++){
			...
			if(allQueuesAreNonEmpty() && inputIsInAllQueuesExactMatch() && !phaseIs("combining")){
				//prepareOutput();
	...
	public void internalTransition()
	{
		...
		removeElementFromAllQueuesExactMatch();
		if(allQueuesAreNonEmpty()) {
			if(inputIsInAllQueuesExactMatch()) {
				total_waiting_time += currentTime - start_waiting;
				//System.out.println("Current total waiting time in node " +this.getName()+ " : " + total_waiting_time);
				holdIn("combining",processing_time);
			} else {
	...
	public MessageBag getOutput()
	{
		...
		if (phaseIs("combining"))
			for(Port entry : this.getOutputPorts()) {
				//for(String j: jobs)
				//m.add(entry,j);
				m.add(entry,job_created_str);//modified to manage duplicates
			}
		return m;
	}
