...
//public class BEHAVIOR2SelectMarkovAtomic extends AtomicModelImpl implements PhaseBased,
public class BEHAVIOR2 extends AtomicModelImpl implements PhaseBased,
StateVariableBased {
	...
	// Input ports
	public final Port<Serializable> inFLOW14 = 
			addInputPort("inFLOW14", Serializable.class);
	// End input ports

	// Output ports
	public final Port<Serializable> outFLOW15 = 
			addOutputPort("outFLOW15", Serializable.class);
	//ENDID
	...
	public BEHAVIOR2(String name) {
		this(name, null);
	}

	public BEHAVIOR2(String name, Simulator simulator) {
	...
	public void initialize() {
		...
		try {
			ctm.fillTransitionInfoList(path + File.separator + "xml" + 
					File.separator + "BEHAVIOR2.xml");
			TransitionInfo ti;
			TimeInfo tf;
			ti = ctm.getTransitionInfoFor("active", "portFLOW15_state");
            tf = ti.getTInfo();
	...
	public void internalTransition() {
		...
		if (phaseIs("portFLOW15_state")) {
			getSimulator().modelMessage("Internal transition from portFLOW15_state");
	...
	public MessageBag getOutput() {
		...
		if (phaseIs("active")) {
			...
			int counter = 0;
			counter++;
			if (ctm.isOutput() && ctm.getNextState().equals("portFLOW15_state")) {
				//output.add(FLOW15,"FLOW15");
				output.add(outFLOW15,job); //Modified to preserve I/O identity
			}
		}
	...
	public static void main(String[] args) {
		...
		//BEHAVIOR2SelectMarkovAtomic model = new BEHAVIOR2SelectMarkovAtomic();
		BEHAVIOR2 model = new BEHAVIOR2();
		model.options = options;

		if (options.isDisableViewer()) { // Command line output only
			Simulation sim =
					new com.ms4systems.devs.core.simulation.impl.SimulationImpl("BEHAVIOR2 Simulation",
							model, options);
	...
	protected static File getModelsDirectory() {
		...
		File dir;
		try {
			//dirUri = BEHAVIOR2SelectMarkovAtomic.class.getResource(".").toURI();
			dirUri = BEHAVIOR2.class.getResource(".").toURI();
			dir = new File(dirUri);
		} catch (URISyntaxException e) {
			e.printStackTrace();
			throw new RuntimeException(
					"Could not find Models directory. Invalid model URL: " +
							//BEHAVIOR2SelectMarkovAtomic.class.getResource(".").toString());
							BEHAVIOR2.class.getResource(".").toString());
		}
	...
	public String[] getPhaseNames() {
		...
		return new String[] { "active"		,"portFLOW15_state"
 };
	}
