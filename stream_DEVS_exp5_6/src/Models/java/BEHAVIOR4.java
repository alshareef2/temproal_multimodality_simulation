...
//public class BEHAVIOR4SelectMarkovAtomic extends AtomicModelImpl implements PhaseBased,
public class BEHAVIOR4 extends AtomicModelImpl implements PhaseBased,
StateVariableBased {
	...
	// Input ports
	public final Port<Serializable> inFLOW16 = 
			addInputPort("inFLOW16", Serializable.class);
	public final Port<Serializable> inFLOW19 = 
			addInputPort("inFLOW19", Serializable.class);
	// End input ports

	// Output ports
	public final Port<Serializable> outFLOW17 = 
			addOutputPort("outFLOW17", Serializable.class);
	//ENDID
	...
	public BEHAVIOR4(String name) {
		this(name, null);
	}

	public BEHAVIOR4(String name, Simulator simulator) {
	...
	public void initialize() {
		...
		try {
			ctm.fillTransitionInfoList(path + File.separator + "xml" + 
					File.separator + "BEHAVIOR4.xml");
			TransitionInfo ti;
			TimeInfo tf;
			ti = ctm.getTransitionInfoFor("active", "portFLOW17_state");
            tf = ti.getTInfo();
	...
	public void internalTransition() {
		...
		if (phaseIs("portFLOW17_state")) {
			getSimulator().modelMessage("Internal transition from portFLOW17_state");
	...
	public MessageBag getOutput() {
		...
		if (phaseIs("active")) {
			...
			int counter = 0;
			counter++;
			if (ctm.isOutput() && ctm.getNextState().equals("portFLOW17_state")) {
				//output.add(FLOW17,"FLOW17");
				output.add(outFLOW17,job); //Modified to preserve I/O identity
			}
		}
	...
	public static void main(String[] args) {
		...
		//BEHAVIOR4SelectMarkovAtomic model = new BEHAVIOR4SelectMarkovAtomic();
		BEHAVIOR4 model = new BEHAVIOR4();
		model.options = options;

		if (options.isDisableViewer()) { // Command line output only
			Simulation sim =
					new com.ms4systems.devs.core.simulation.impl.SimulationImpl("BEHAVIOR4 Simulation",
							model, options);
	...
	protected static File getModelsDirectory() {
		...
		File dir;
		try {
			//dirUri = BEHAVIOR4SelectMarkovAtomic.class.getResource(".").toURI();
			dirUri = BEHAVIOR4.class.getResource(".").toURI();
			dir = new File(dirUri);
		} catch (URISyntaxException e) {
			e.printStackTrace();
			throw new RuntimeException(
					"Could not find Models directory. Invalid model URL: " +
							//BEHAVIOR4SelectMarkovAtomic.class.getResource(".").toString());
							BEHAVIOR4.class.getResource(".").toString());
		}
	...
	public String[] getPhaseNames() {
		...
		return new String[] { "active"		,"portFLOW17_state"
 };
	}
