...
//public class BEHAVIOR5SelectMarkovAtomic extends AtomicModelImpl implements PhaseBased,
public class BEHAVIOR5 extends AtomicModelImpl implements PhaseBased,
StateVariableBased {
	...
	// Output ports
	public final Port<Serializable> outFLOW20 = 
			addOutputPort("outFLOW20", Serializable.class);
	//ENDID
	...
	public BEHAVIOR5(String name) {
		this(name, null);
	}

	public BEHAVIOR5(String name, Simulator simulator) {
	...
	public void initialize() {
		...
		try {
			ctm.fillTransitionInfoList(path + File.separator + "xml" + 
					File.separator + "BEHAVIOR5.xml");
			TransitionInfo ti;
			TimeInfo tf;
			ti = ctm.getTransitionInfoFor("active", "portFLOW20_state");
            tf = ti.getTInfo();
	...
	public void internalTransition() {
		...
		if (phaseIs("portFLOW20_state")) {
			getSimulator().modelMessage("Internal transition from portFLOW20_state");
	...
	public MessageBag getOutput() {
		...
		if (phaseIs("active")) {
			...
			int counter = 0;
			counter++;
			if (ctm.isOutput() && ctm.getNextState().equals("portFLOW20_state")) {
				//output.add(FLOW20,"FLOW20");
				output.add(outFLOW20,job); //Modified to preserve I/O identity
			}
		}
	...
	public static void main(String[] args) {
		...
		//BEHAVIOR5SelectMarkovAtomic model = new BEHAVIOR5SelectMarkovAtomic();
		BEHAVIOR5 model = new BEHAVIOR5();
		model.options = options;

		if (options.isDisableViewer()) { // Command line output only
			Simulation sim =
					new com.ms4systems.devs.core.simulation.impl.SimulationImpl("BEHAVIOR5 Simulation",
							model, options);
	...
	protected static File getModelsDirectory() {
		...
		File dir;
		try {
			//dirUri = BEHAVIOR5SelectMarkovAtomic.class.getResource(".").toURI();
			dirUri = BEHAVIOR5.class.getResource(".").toURI();
			dir = new File(dirUri);
		} catch (URISyntaxException e) {
			e.printStackTrace();
			throw new RuntimeException(
					"Could not find Models directory. Invalid model URL: " +
							//BEHAVIOR5SelectMarkovAtomic.class.getResource(".").toString());
							BEHAVIOR5.class.getResource(".").toString());
		}
	...
	public String[] getPhaseNames() {
		...
		return new String[] { "active"		,"portFLOW20_state"
 };
	}
